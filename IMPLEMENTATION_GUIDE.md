...
- Gemini は呼んだ時だけ。質問・単純操作は**小型安価モデル（Flash / Flash-Lite）**、重い判断（計画・コード生成・Web 調査を伴う複合操作・直接 state 編集）は上位モデル＝**モデルルーティング**。
...
提出版(7/10)に **案1・案3・案4 を含める**。**案2（BYOK）は MVP から外し、セルフホスト用ロードマップに降格**（理由は §2.6「案2 補足」）。
...
- 共通IF `LLMProvider.generate(messages, schema?, tier?)` に集約。登録プロバイダ：
  - `gemini`（**本番既定**・アプリ鍵）
  - `gemini-byok`（**セルフホスト用・ロードマップ**。ユーザーのGemini鍵をサーバ側で使う。共有ホスト版では採用しない＝下記「案2 補足」）
...
- 本番既定は Gemini を維持（必須要件②）。質問・単純操作は小型(Flash)、コード生成・Web 調査を伴う複合操作・直接 state 編集は上位(PRO)とするモデルルーティングを Gateway 経由で実施。

#### 案2 補足：BYOK は MVP から外し、セルフホスト用に降格（2026-06-09 決定）
- **理由（運営者トラスト問題）**：server-side BYOK では、LLM 呼び出し時に平文鍵が必要なため、サーバ＝運営者が他人の鍵を技術的に悪用できる。Secret Manager/暗号化は外部侵入・DB漏洩は防ぐが**運営者の悪用は防げない**。
- **方針**：**共有ホスト版（コンテスト）＝アプリ管理の Gemini 鍵のみ。第三者の鍵は一切保管しない**。BYOK は**セルフホスト**（自分のインスタンス＝運営者は本人）向けロードマップ。これはセキュリティ成熟度として審査基準5でアピール可。
- **重要**：鍵の話とデータ隔離（案1）は別レイヤ。アプリ管理鍵を全ユーザー共用しても、データは uid で完全分離できる（案1は鍵と独立に成立）。
