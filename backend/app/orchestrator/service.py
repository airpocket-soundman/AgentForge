...
def plan_and_register(req: PlanRequest, design_plan: dict | None = None) -> PlanResponse:
    ...
    Every feature is really generated by the UI Designer worker (no hard-coded
    screens). When `design_plan` is given (the proposal the user reviewed and
    approved), the worker implements that exact plan.

    The WorkPlan skeleton here is built DETERMINISTICALLY (no LLM): the approved
    design proposal already captured the design, and the views/feature/theme are
    overwritten from the generated manifest below. The single heavy (PRO) call in
    this path is `ui_designer.design` — we do not spend a second PRO call
    re-planning (that output was discarded anyway)."""
    task_id = f"task_{uuid.uuid4().hex[:12]}"
    feature = req.feature or infer_feature(req.goal)
    plan = _stub_plan(task_id, req, feature)
    if _wants_no_worker(req.goal):
    ...
    manifest = ui_designer.design(req.goal, plan=design_plan)
