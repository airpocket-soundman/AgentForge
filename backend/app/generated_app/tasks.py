...
@router.get("")
def list_tasks(project_id: str = "default", user: CurrentUser = Depends(current_user)) -> dict:
...
@router.post("")
def create_task(body: TaskIn, user: CurrentUser = Depends(current_user)) -> Task:
...
def post_task_message(task_id: str, body: TaskMessageIn, user: CurrentUser = Depends(current_user)) -> dict:
...
def update_task(task_id: str, body: TaskUpdate, user: CurrentUser = Depends(current_user)) -> Task:
    ref = get_db().collection(_COLLECTION).document(task_id)
    snap = ref.get()
    if not snap.exists:
        raise HTTPException(status_code=404, detail="task が見つかりません")
    current = snap.to_dict()
    require_project_access(user, current["project_id"])
