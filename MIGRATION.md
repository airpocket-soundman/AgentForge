...
## 6. 留意・未決
- **ゲートへの生成物の渡し方（決定）：インライン**（プロンプトに HTML をそのまま埋め込む）。本番 Gemini はローカルファイルパスを読めず内容はインラインが前提のため、**テスト環境も本番に準拠してインライン**で統一する。claude-cli 専用のファイルパス渡し最適化は**採用しない**（環境差を作らない）。※ プロンプト肥大による誤検知防止のため、HTML は丸ごと送る（スライス禁止）。
